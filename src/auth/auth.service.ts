...
export class AuthService {
    ...
    async validateUser(email: string, password: string): Promise<any> {
        ...
        if (!user.password) {
            console.log(`[AuthService] Login failed: User exists but has no password (OAuth user?) (${email})`);
            return null;
        }
    ...
    async resetPassword(token: string, newPassword: string) {
        ...
        const users = await this.userRepository.find({
            where: {
                resetPasswordExpires: new Date() as any, // TypeORM will handle the comparison
            },
        });
        ...
        for (const u of users) {
            if (u.resetPasswordToken && u.resetPasswordExpires && u.resetPasswordExpires > new Date()) {
