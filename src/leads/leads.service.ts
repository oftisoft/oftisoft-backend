...
export class LeadService {
    ...
    async getStats() {
        ...
        const newLeads = await this.leadRepository.count({ where: { status: LeadStatus.NEW } });
        const ctaCount = await this.leadRepository.count({ where: { type: LeadType.CTA } });
        const newsletterCount = await this.leadRepository.count({ where: { type: LeadType.NEWSLETTER } });
