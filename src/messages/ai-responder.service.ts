...
Current Message: `;
    ...
    async generateResponse(userMessage: string): Promise<string> {
        ...
        if (msg.includes('service') || msg.includes('provide') || msg.includes('do you do')) {
        ...
        if (msg.includes('experience') || msg.includes('rasel') || msg.includes('who are you')) {
        ...
        if (msg.includes('dashboard') || msg.includes('login') || msg.includes('access')) {
    ...
    async getOrCreateAIBot(): Promise<User> {
        let bot = await this.usersRepo.findOne({ where: { isAI: true, email: 'sarah@oftisoft.com' } });
        ...
        if (!bot) {
            bot = this.usersRepo.create({
                ...
                role: 'Support',
                isActive: true,
                avatarUrl: 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?q=80&w=256&h=256&auto=format&fit=crop', // A professional woman's face
            });
