...
export class OrdersController {
  ...
  async downloadInvoice(
    ...
    const buffer = await this.ordersService.generateInvoice(
      req.user.userId,
      id,
    );
    res.set({
  ...
  async updateOrder(
    @Param('id') id: string,
    @Body() body: { internalNotes?: string; trackingNumber?: string },
    @Req() req,
  ) {
    return this.ordersService.updateOrder(req.user.userId, id, body);
  }
}
