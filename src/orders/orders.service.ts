...
export class OrdersService {
    ...
    async updateStatus(userId: string, id: string, status: string): Promise<Order> {
    ...
    async generateInvoice(userId: string, id: string): Promise<Buffer> {
        ...
        return new Promise((resolve) => {
            ...
            // Header
            doc.fontSize(20).text(`Invoice #${order.id.slice(0, 8)}`, { align: 'right' });
            ...
            if (order.shippingAddress) {
                ...
                doc.text(`${order.shippingAddress.country}, ${order.shippingAddress.zip}`);
            }
            doc.moveDown();
            ...
            order.items.forEach(item => {
                ...
                doc.text(`$${Number(item.price).toFixed(2)}`, 400, y, { width: 90, align: 'right' });
                y += 20;
            });
            ...
            // Total
            doc.fontSize(14).text(`Total: $${Number(order.total).toFixed(2)}`, 400, y + 30, { width: 90, align: 'right' });
    ...
    async generateReport(userId: string): Promise<string> {
        ...
        return new Promise((resolve, reject) => {
            stringify(data, {
                header: true,
                columns: ['Order ID', 'Date', 'Status', 'Items Count', 'Total Amount', 'Payment Method']
            }, (err, output) => {
                if (err) reject(err);
                else resolve(output);
            });
